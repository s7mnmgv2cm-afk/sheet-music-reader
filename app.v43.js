...
const els = {
    ...
    ignoreOctaveCheckbox: document.getElementById('ignore-octave-checkbox'),
    ...
    pianoWrapper: document.querySelector('.piano-wrapper'),
...
function setupEventListeners() {
    ...
    if (els.ignoreOctaveCheckbox) {
        els.ignoreOctaveCheckbox.addEventListener('change', (e) => {
            if (e.target.checked) {
                els.pianoWrapper.classList.add('single-octave');
            } else {
                els.pianoWrapper.classList.remove('single-octave');
            }
        });
    }
    ...
    // Piano Interaction events
        ...
        const releaseKey = (e) => {
            ...
            if (synth && state.appMode !== 'practice') {
        ...
        // Track touch to prevent synthetic mouse events
        let lastTouchTime = 0;

        // Standard Mouse Events
        keyEl.addEventListener('mousedown', (e) => {
            if (e.button !== 0) return; // Only respond to left click
            if (Date.now() - lastTouchTime < 500) return; // Ignore synthetic mousedown after touch
            pressKey(e);
        });
        keyEl.addEventListener('mouseup', releaseKey);
        keyEl.addEventListener('mouseleave', releaseKey);
        
        // Standard Touch Events (allow default so user can scroll horizontally!)
        ...
        keyEl.addEventListener('touchcancel', releaseKey, {passive: true});
    });
...
function checkAnswer() {
    ...
    const ignoreOct = els.ignoreOctaveCheckbox && els.ignoreOctaveCheckbox.checked;

    if (isChord) {
        if (state.currentChordInput.length < activeItem.length) return;
        
        const correctPitches = activeItem.map(n => getAbsolutePitch(n.key, n.accidental, n.octave));
        const selectedPitches = state.currentChordInput.map(n => getAbsolutePitch(n.note, n.accidental, n.octave));
        
        if (ignoreOct) {
            const correctMods = correctPitches.map(p => (p % 12 + 12) % 12).sort((a,b)=>a-b);
            const selectedMods = selectedPitches.map(p => (p % 12 + 12) % 12).sort((a,b)=>a-b);
            isCorrect = correctMods.length === selectedMods.length && 
                        correctMods.every((p, i) => p === selectedMods[i]);
        } else {
            correctPitches.sort((a,b)=>a-b);
            selectedPitches.sort((a,b)=>a-b);
        ...
        if (ignoreOct) {
            isCorrect = ((correctPitch % 12 + 12) % 12) === ((selectedPitch % 12 + 12) % 12);
        } else {
...
function renderSong() {
    ...
    // Create Fixed Hit Line
    const hitLine = document.createElement('div');
    hitLine.style.position = 'absolute';
    hitLine.style.left = '150px';
    hitLine.style.top = '10px';
    hitLine.style.bottom = '10px';
    hitLine.style.width = '4px';
    hitLine.style.backgroundColor = 'rgba(52, 152, 219, 0.8)';
    hitLine.style.borderRadius = '2px';
    hitLine.style.zIndex = '20';
    hitLine.style.boxShadow = '0 0 10px rgba(52, 152, 219, 0.5)';
    els.staffContainer.appendChild(hitLine);
    
    // Create Hit Feedback text
    const hitFeedback = document.createElement('div');
    hitFeedback.id = 'hit-feedback';
    hitFeedback.style.position = 'absolute';
    hitFeedback.style.left = '150px';
    hitFeedback.style.top = '10px';
    hitFeedback.style.transform = 'translateX(-50%)';
    hitFeedback.style.fontWeight = 'bold';
    hitFeedback.style.fontSize = '24px';
    hitFeedback.style.zIndex = '30';
    hitFeedback.style.pointerEvents = 'none';
    hitFeedback.style.transition = 'opacity 0.2s, transform 0.2s';
    hitFeedback.style.opacity = '0';
    els.staffContainer.appendChild(hitFeedback);
    els.hitFeedback = hitFeedback;
    
    // Create SVG Wrapper that will scroll
    const svgWrapper = document.createElement('div');
    svgWrapper.style.position = 'absolute';
    svgWrapper.style.left = '0';
    svgWrapper.style.top = '0';
    svgWrapper.style.height = '100%';
    svgWrapper.style.willChange = 'transform'; // Hardware acceleration
    els.staffContainer.appendChild(svgWrapper);
    els.svgWrapper = svgWrapper;
    
    // Set container to block layout so absolute positioning works perfectly
    els.staffContainer.style.display = 'block';
    els.staffContainer.style.overflow = 'hidden'; 

    const VF = Vex.Flow;
    const renderer = new VF.Renderer(svgWrapper, VF.Renderer.Backends.SVG);
    
    const measureWidth = 250;
    const totalWidth = (state.songData.measures.length * measureWidth + 50) * 1.5;
    renderer.resize(totalWidth, 180 * 1.5);
    const context = renderer.getContext();
...
function startSong() {
    ...
    state.songStartTime = Tone.now() + (countInBeats * (60 / bpm));
    
    // Schedule count-in metronome ticks
    for (let i = 0; i < countInBeats; i++) {
        synth.triggerAttackRelease("C5", "32n", Tone.now() + i * (60 / bpm), 0.1);
    }
    
    scheduleSong();
    
    // Start Transport at the exact time the song starts
    Tone.Transport.start(state.songStartTime);
    
    requestAnimationFrame(drawTick);
}

function scheduleSong() {
    els.svgWrapper.style.transition = 'none'; // We animate manually
...
function drawTick() {
    try {
        ...
        // Calculate exact current beat based on wall clock time
        const now = Tone.now();
        const currentBeat = (now - state.songStartTime) * bps;
        
        let currentX = 15;
        
        if (currentBeat < 0) {
            // Count-in phase
            const firstNote = state.songNotesTimeline[0];
            const secondNote = state.songNotesTimeline[1];
            const startX = firstNote.x * 1.5 + 15;
            let pixelsPerBeat = 100; // fallback
            if (secondNote) {
                pixelsPerBeat = (secondNote.x * 1.5 + 15 - startX) / (secondNote.timeInBeats - firstNote.timeInBeats);
            }
            currentX = startX + currentBeat * pixelsPerBeat;
            ...
            if (nextNote) {
                ...
                currentX = startX + (endX - startX) * beatProgress;
            } else {
                // Last note ending
                if (currentBeat >= currentNote.timeInBeats + currentNote.durationBeats) {
                    setTimeout(stopSong, 1000); 
                    state.songIsPlaying = false;
                    return;
                }
                const beatProgress = (currentBeat - currentNote.timeInBeats) / currentNote.durationBeats;
                const startX = currentNote.x * 1.5 + 15;
                currentX = startX + 100 * beatProgress;
            }
        }
        
        // Align currentX exactly with the hitLine (150px)
        const shiftX = 150 - currentX;
        els.svgWrapper.style.transform = `translateX(${shiftX}px)`;
        
        requestAnimationFrame(drawTick);
    } catch(e) {
        console.error("Error in drawTick:", e);
    }
}
...
function showHitFeedback(text, color) {
    if (!els.hitFeedback) return;
    els.hitFeedback.textContent = text;
    els.hitFeedback.style.color = color;
    els.hitFeedback.style.opacity = '1';
    els.hitFeedback.style.transform = 'translateX(-50%) translateY(-20px)';
    
    // Reset animation
    setTimeout(() => {
        if (!els.hitFeedback) return;
        els.hitFeedback.style.opacity = '0';
        els.hitFeedback.style.transform = 'translateX(-50%) translateY(0)';
    }, 400);
}
...
function handleSongHit(note, accidental, octave) {
    ...
    // Rhythm Game Hit Logic
    if (closestNote && minDiff < 0.6) {
        const getAbsolutePitch = (n, acc, oct) => {
            const basePitches = { 'c': 0, 'd': 2, 'e': 4, 'f': 5, 'g': 7, 'a': 9, 'b': 11 };
        ...
        if (correctPitch === selectedPitch) {
            ...
            let feedbackColor = "#f1c40f"; // Gold
            if (minDiff > 0.15 && minDiff <= 0.3) {
                feedbackText = "Great!";
                feedbackColor = "#2ecc71"; // Green
            } else if (minDiff > 0.3) {
                feedbackText = "Good";
                feedbackColor = "#3498db"; // Blue
            }
            showHitFeedback(feedbackText, feedbackColor);
            
            closestNote.staveNote.setStyle({fillStyle: feedbackColor, strokeStyle: feedbackColor});
            closestNote.staveNote.draw();
        } else {
            state.songCombo = 0;
            els.songScore.textContent = `Combo: ${state.songCombo}`;
            showHitFeedback("Miss!", "#e74c3c");
            closestNote.staveNote.setStyle({fillStyle: "#e74c3c", strokeStyle: "#e74c3c"});
            closestNote.staveNote.draw();
        }
    } else if (minDiff >= 0.6) {
        showHitFeedback("Way Off!", "#95a5a6");
    }
}
