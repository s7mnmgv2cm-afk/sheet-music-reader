...
function setupEventListeners() {
    ...
    els.pianoKeys.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (synth) {
                // Construct the note name for Tone.js (e.g., C4, C#4)
                let toneAccidental = state.selectedAnswer.accidental === 'n' ? '' : state.selectedAnswer.accidental;
                // We'll just play it in octave 4 for feedback
                synth.triggerAttackRelease(`${state.selectedAnswer.note}${toneAccidental}4`, "8n");
            }
        });
    });
